/* 탭메뉴 구성
 1 메뉴 셀렉트의 메뉴중 4가지 메뉴를 구성해둠
 맨 처음화면은 mainlist를 보여주고 나머지 리스트는 display none을 적용한 클래스를 하나 만들어서
 해당클래스를 적용시킴
 선택한 메뉴만 active 클래스가 적용되고 나머지는 전부 none클래스로 변경
 2. 각 메뉴와 그 메뉴에 해당하는 main  water buger snack을 변수로 지정
 */

 document.addEventListener('DOMContentLoaded',function(){
var mainmenu = document.querySelectorAll('.taptap')
var mainlist = document.querySelectorAll('.fullmenu section')

mainmenu.forEach(function(item, idx){
    item.addEventListener('click',function(){
        for(i=0; i<mainmenu.length; i++){
            if(idx == i){
                mainlist[idx].classList.remove('none') 
            }else{
                mainlist[i].classList.add('none')
            }
        }
    })
})
...
/*
    주소창 누르면 이벤트 발생
    find 창이 display fixed 로 나타나고
    closebox 클릭시 find창이 display none이 되도록 하면 될것같음
    먼저 경고창이 한번 뜨는것 같음 로그인이 필요합니다 라고
    매장찾기는 이미 이벤트가 하나 있긴 함 

*/
var mainmenu5 = document.querySelector('.menuselect>ul>li:last-child')
var closebox = document.querySelector('.closebox')
var find1 = document.querySelector('.find')
    find1.style.display = 'none'

mainmenu5.addEventListener('click',function(){
    console.log('클릭')
